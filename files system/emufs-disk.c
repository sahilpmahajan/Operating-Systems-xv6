...
int alloc_inode(int mount_point){
	...
	*/
	...
	if(superblock.used_inodes == MAX_INODES) return -1;
...
void read_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	struct mount_t mount = mounts[mount_point];
	int blocknum;
	if(inodenum < 16) blocknum = 1;
	else blocknum = 2;
	readblock(mount.device_fd,blocknum,(char*)&metadata);
	// decrypt(mount.key,(char*)&metadata,sizeof(metadata));
	memcpy(inodeptr,&metadata.inodes[inodenum%16],sizeof(struct inode_t));
}
...
void write_inode(int mount_point, int inodenum, struct inode_t *inodeptr){
	...
	*/
	...
	readblock(mount.device_fd,blocknum,(char*)&metadata);
	// decrypt(mount.key,(char*)&metadata,sizeof(metadata));
	memcpy(&metadata.inodes[inodenum%16],inodeptr,sizeof(struct inode_t));
	// encrypt(mount.key,(char*)&metadata,sizeof(metadata));
	writeblock(mount.device_fd,blocknum,(char*)&metadata);
}
...
int alloc_datablock(int mount_point){
	...
	*/
	struct superblock_t superblock;
	read_superblock(mount_point,&superblock);
	if(superblock.used_blocks == MAX_BLOCKS) return -1;
...
void read_datablock(int mount_point, int blocknum, char *buf){
	...
	*/
	struct mount_t mount = mounts[mount_point];
	readblock(mount.device_fd,blocknum,buf);
	// decrypt(mount.key,buf,strlen(buf));
}
...
void write_datablock(int mount_point, int blocknum, char *buf){
	...
	*/
	struct mount_t mount = mounts[mount_point];
	// encrypt(mount.key,buf,strlen(buf));
	writeblock(mount.device_fd,blocknum,buf);
}